...
//  EyesTracker
//
//  Created by Rubanov on 6/11/18.
//  Copyright © 2018 Rubanov. All rights reserved.
//
...
class ViewController: UIViewController, ARSCNViewDelegate {
   ...
   private struct Constants {
        static let GroupID = "group.rubanov.tracking"
    ...
    private func setupSliderPortal() {
        sliderPortal = BBPortal(withGroupIdentifier: Constants.GroupID, andPortalID: "portal")
    ...
    func sendData(data: String) {
        self.sliderPortal?.send(data: [Constants.SliderKey: data], onCompleted: { (error) in
...
//        setupPortals()
        ...
        turnOnButton.clipsToBounds = true
        turnOnButton.layer.cornerRadius = 15
        turnOnButton.setTitle("Turn on",for: .normal)
        turnOnButton.backgroundColor = .black
        turnOnButton.frame = CGRect(x: 0, y: 0, width: self.view.frame.width, height: self.view.frame.height / 2)
        turnOnButton.alpha = 0.4
        sceneView.addSubview(turnOnButton)
        ...
        minusButton.frame = CGRect(x: 0, y: self.view.frame.height / 2, width: self.view.frame.width / 2, height: self.view.frame.height / 2)
        ...
        plusButton.frame = CGRect(x: self.view.frame.width / 2, y: self.view.frame.height / 2, width: self.view.frame.width / 2, height: self.view.frame.height / 2)
        plusButton.alpha = 0.4
        sceneView.addSubview(turnOnButton)
        ...
        Timer.scheduledTimer(withTimeInterval: 1, repeats: true) { (timer) in
            if self.target.frame.origin.y < self.view.frame.height / 2 && self.target.frame.origin.y > 0  {
                check += 1
                turnOnButton.setTitle("time: \(check)",for: .normal)
                if check == 3 {
                    if sost == 1 {
                        self.sendData(data: "Turn on")
                        UIView.animate(withDuration: 1, animations: {
                            turnOnButton.setTitle("Turn off",for: .normal)
                            sost = 0
                            turnOnButton.alpha = 0.8
                        })
                        check = 0
                    } else {
                        self.sendData(data: "Turn off")
                        UIView.animate(withDuration: 1, animations: {
                            turnOnButton.setTitle("Turn on",for: .normal)
                            sost = 1
                            turnOnButton.alpha = 0.4
